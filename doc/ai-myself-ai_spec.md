...
3. 実装
   実装はいきなり詳細ロジックを実装せず、最初バージョン（バージョン 1）では、フロントエンドとバックエンド双方の関数や API、イベントハンドラ、RPC のスケルトン作成とそれらの間の呼び出し処理の実装をしてください。特に、Firebase Emulator Suite を使用したローカル開発環境において、フロントエンドが各エミュレータ（Authentication, Functions, Firestore）に正しく接続し、認証や API 呼び出しが完結することを確認できるように実装してください。また、今後のデバッグを容易にするために呼び出し前後のエラー判定、例外処理の実装やログ機能の実装をしてください。その内容で一度ローカル環境とデプロイ環境で動作確認を完了してから次のバージョンの作成に移るため、バージョン 1 として付帯成果物も更新してください。
