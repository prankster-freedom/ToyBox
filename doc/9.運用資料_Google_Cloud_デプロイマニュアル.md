...
「Google Cloud 環境初期設定マニュアル」の「6. Cloud Build トリガーの作成」に従い、トリガーが設定されていることを確認してください。
...
また、API レスポンスへのデバッグログ出力は `ENABLE_DEBUG_LOGS` 環境変数で制御されます（デフォルト: `true`）。
本番環境でデバッグログを無効化したい場合は、`ENABLE_DEBUG_LOGS=false` を設定してください。

その他の環境変数（API キーなど）が必要な場合は、Cloud Run のコンソールまたは `gcloud` コマンドで設定してください。

```bash
gcloud run services update toybox-service \
  --update-env-vars KEY=VALUE,ENABLE_DEBUG_LOGS=false
```

また、Datastore への接続を確実にするため、`GOOGLE_CLOUD_PROJECT` 環境変数の設定が必要です（ライブラリによる自動検出が不安定な場合があるため）。
