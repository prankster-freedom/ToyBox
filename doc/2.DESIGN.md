...
- **Frontend (`public/`)**: クライアントサイドのレンダリングとユーザーインタラクションを担当する SPA。
  - `index.html`: 一般ユーザー向けアプリケーションのエントリーポイント。
  - `script.js`: 一般ユーザー向けアプリケーションのバックエンド API との通信、DOM 操作を担当。
  - `developer.html`: 開発者向け API コンソールのエントリーポイント。
  - `developer.js`: 開発者向け API コンソールのバックエンド API との通信、DOM 操作、デバッグ情報の表示を担当。
- **Backend**: サーバーサイドのロジック、API 提供、認証を担当。
  ...
  - `routes/`: 各 API エンドポイントの定義。
    ...
    - `posts.js`: 投稿関連のAPIルート (`/api/posts`) を担当。
    - `greet.js`: 挨拶機能のAPIルート (`/api/greet`) を担当。
    - `health.js`: ヘルスチェックのAPIルート (`/api/health`) を担当。
    - `chat.js`: AIチャットのAPIルート (`/api/chat`) を担当。
  - `middleware/`: Express のミドルウェア。
    - `auth.js`: 認証ミドルウェア (`isAuthenticated`) を担当。
    - `requestLogger.js`: リクエスト単位でログを収集・管理するミドルウェアを担当。
    - `responseLogger.js`: レスポンスにデバッグログを注入するミドルウェアを担当。
  - `lib/`: 再利用可能なビジネスロジック。
    - `posts.js`: `data/posts.json` への読み書きをカプセル化するデータアクセス層。
    - `ai.js`: 外部のAIエンジン (Google AI) との通信をカプセル化する。
    - `logger.js`: `AsyncLocalStorage` を利用したリクエスト単位のコンテキストを持つロガーを提供する。
...
sequenceDiagram
    ...
    note over User, Google: このシーケンスは本番環境 (production mode) のものです。<br>開発環境 (development mode) では、認証はバイパスされ、<br>モックユーザーが自動的に使用されます。
...
sequenceDiagram
    ...
    participant Frontend
    participant Backend
    participant AIEngine as Google AI

    User->>Frontend: チャットメッセージを送信
    Frontend->>Backend: `POST /api/chat` を呼び出す (認証情報と共に)
    Backend->>AIEngine: メッセージを整形してAPIリクエストを送信
    AIEngine-->>Backend: AIからの応答を返す
    Backend-->>Frontend: 応答メッセージをJSONで返す
    Frontend-->>User: AIからの応答を表示
```

### 6.3. 開発者向けUI API実行フロー

```mermaid
sequenceDiagram
    participant Developer
    participant DevFrontend as "Developer UI (developer.html)"
    participant Backend

    Developer->>DevFrontend: API実行ボタンをクリック
    DevFrontend->>Backend: `fetchApi()` を実行し、選択されたAPIを呼び出す
    note right of Backend: 開発モードの場合、<br>リクエストコンテキストで<br>ログ収集が開始される
    Backend->>Backend: APIの内部処理を実行
    note right of Backend: 処理の各所でログが記録される
    Backend-->>DevFrontend: JSONレスポンスを返す<br>(_debugフィールドに収集したログを含む)
    DevFrontend->>DevFrontend: レスポンスから `_debug` 情報を分離
    DevFrontend-->>Developer: APIの主レスポンスと、<br>整形されたデバッグログを画面に表示
```
...
### 8.2. 設定管理
- Google AIのAPIキーのような外部サービス連携に必要な機密情報も、同様に環境変数で管理します。
...
- **`development` モード:**
  ...
  - 認証がバイパスされます。保護されたAPIへのリクエストは、常に認証済みのモックユーザーからのものとして扱われます。これにより、フロントエンド開発者がバックエンドの認証フローを意識することなく開発を進められます。
...
- **AIエンジン**: `SPEC.md`の要件を満たすため、Google AI PlatformのGeminiモデルを利用します。APIとの通信は`lib/ai.js`にカプセル化し、他のコンポーネントへの依存を最小限に抑えます。
...
- **技術的負負債: フロントエンドのモノリシックな構造**
  - 現在のフロントエンドは、`script.js` や `developer.html` 内のインラインスクリプトなど、複数のファイルにロジックが分散・重複しています。
  - 今後、`SPEC.md`で要求されている一般ユーザー向けUIと開発者向けUIを実装していくにあたり、コンポーネント分割などの適切な構造化を行わない限り、保守性が著しく低下します。
  - **解決策案:**
    - `developer.html` からインラインスクリプトを `public/developer.js` に分離する。
