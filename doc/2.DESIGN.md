...
- **開発言語:** フロントエンド (Vanilla JS), バックエンド (Node.js)
...
```plantuml
@startuml
title システムコンテキスト (Components)
skinparam linetype ortho
skinparam componentStyle uml2

actor User

package "ToyBox Application" {
    component "Vue.js SPA" as Frontend

    package "Cloud Run (Compute)" as CloudRun {
        component "Node.js/Express API" as Backend
        component "Cloud Run Job (Batch)" as Job
    }
}

package "Platform Services" {
    database "Cloud Datastore" as Datastore
    component "Cloud Tasks" as Tasks
    component "Cloud Scheduler" as Scheduler
    component "Google Identity Services" as GoogleAuth
}
...
CloudRun --> Datastore : Read/Write
CloudRun --> Gemini : Generate Content

' Specific Interactions
Backend --> GoogleAuth : OAuth 2.0
Backend <--> Tasks : Enqueue / Trigger
...
- **DS02: 非同期処理とバッチ処理**
  - `daydream` (短期記憶の分析) のトリガー判定には、Datastore 上の AiPersonas.interactionCount を利用し、チャット毎にインクリメント・判定を行う。処理自体は **Cloud Tasks** にキューイングし、非同期に実行します。分析には直近の「対話履歴」に加え、ユーザーがこれまでに作成した全ての「想い出（投稿内容）」も入力として利用します。
  - `dream` (長期記憶の更新) は、**Cloud Scheduler** をトリガーとして **Cloud Run Job** で実行します。その日の会話と「これまでの想い出（投稿内容）」を統合して性格を更新します。
- **DS03: AI モデル**
...
- **DS08: モバイル表示最適化戦略 (Refined)**
  - Viewport: `width=device-width, initial-scale=1.0, maximum-scale=5.0, shrink-to-fit=no`
  - オーバーフロー制御: `html`, `body` に `overflow-x: hidden` を適用し、かつ `body { width: 100% }` を設定して不測の横揺れや自動ズームを防止。
  - 背景クリッピング: アニメーション要素を含む背景コンテナ (`.flower-bg`) に `overflow: hidden` を適用。
  - レスポンシブレイアウト: メディアクエリ（`600px` 等）を用いて、モバイル環境下でのパディングやカード幅を自動調整。
- **DS09: 3D UI におけるスクロール制御**
  - 物理的なスクロールバーを使用せず、JavaScript の `scrollOffset` 状態によって 3D 空間内のカード描画位置を動的に制御します。ホイール、スワイプ、フリック操作をこの状態量に変換して反映します。

## 5. ビルディングブロックビュー

### 5.1. コンポーネントの責務

- **Frontend (`public/`)**: Vanilla JS + CSS (参照: `doc/3.detailed_design/frontend_prototype_v1.html`) による SPA。
- **Backend**:
  ...
  - `routes/tasks.js` (New): Cloud Tasks からのトリガー、および開発者テスト用の手動トリガーを受け取るエンドポイント。
...
graph TD
    ...
    Auth -->|Next| ChatRoute
    Auth -->|Next| StoreRoute

    ChatRoute -->|Generate Response| AI
    ChatRoute -->|Save History| Store
    StoreRoute -->|CRUD| Store
```
...
`ENABLE_DEBUG_LOGS` 環境変数が `true` (デフォルト) の場合、そのリクエスト処理中にバックエンドで発生したデバッグログは、レスポンスの一部としてフロントエンドに転送されます。
...
本アプリケーションは、`NODE_ENV` および `IS_LOCAL` 環境変数によって制御されます。

- **`IS_LOCAL` (ローカル環境判定):**

  - `true` の場合、ローカル開発環境として扱われます。
  - `express-session` の `cookie.secure` が `false` に設定され、HTTP 経由でのセッションが許可されます。
  - `true` の場合、ローカル開発環境として扱われます。
  - `express-session` の `cookie.secure` が `false` に設定され、HTTP 経由でのセッションが許可されます。
  - **認証の挙動**: `isAuthenticated` ミドルウェアによる自動バイパスは行いません。代わりに、`/auth/google` エンドポイントがモックユーザーでのログイン処理を行います（Google 認証画面へのリダイレクトをスキップ）。これにより、ログイン/ログアウトのサイクルをローカルでシミュレート可能です。

- **`NODE_ENV` (Node.js 実行モード):**

  - `production` または `development` を設定します。
  - ライブラリの最適化などに影響します。

- **`ENABLE_DEBUG_LOGS` (デバッグログ制御):**
  - `true` (デフォルト) の場合、API レスポンスにデバッグログが含まれます。
  - 本番環境 (`production`) でも、このフラグが `false` でない限りログは出力されます。セキュリティ上の理由から、本番環境では `false` に設定することを推奨します。
