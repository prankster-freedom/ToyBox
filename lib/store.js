...
if (process.env.GOOGLE_CLOUD_PROJECT) {
  ...
  console.log(`Datastore initialized with projectId: ${options.projectId}`);
}
...
export async function getAiPersona(userId) {
  ...
  return (
    entity || {
      basePersonality: "あなたは親切なAIアシスタントです。",
      interactionCount: 0,
    }
  );
}
...
export async function updateAiPersona(userId, data) {
  ...
  const entity = {
    ...
    excludeFromIndexes: ['basePersonality'],
  };
  await datastore.save(entity);
  exit(functionName);
}
...
export async function getRecentAnalyses(userId, limit = 5) {
  ...
  enter(functionName, { userId });
  const query = datastore
    .createQuery(KIND_PERSONALITY_ANALYSIS)
    .filter("user_uid", "=", userId)
    .order("timestamp", { descending: true })
    .limit(limit);
...
export async function deleteUserData(userId) {
  ...
  // ChatMessages
  const msgQuery = datastore
    .createQuery(KIND_CHAT_MESSAGE)
    .filter("user_uid", "=", userId)
    .select("__key__");
  const [msgs] = await datastore.runQuery(msgQuery);
  if (msgs.length > 0)
    await datastore.delete(msgs.map((e) => e[datastore.KEY]));

  // PersonalityAnalyses
  const analysisQuery = datastore
    .createQuery(KIND_PERSONALITY_ANALYSIS)
    .filter("user_uid", "=", userId)
    .select("__key__");
  const [analyses] = await datastore.runQuery(analysisQuery);
  if (analyses.length > 0)
    await datastore.delete(analyses.map((e) => e[datastore.KEY]));
